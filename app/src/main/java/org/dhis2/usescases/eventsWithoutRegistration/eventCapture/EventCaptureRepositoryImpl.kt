...
class EventCaptureRepositoryImpl(
    ...
    override fun hasSimprintsRampProgramStageHistoryTable(): Boolean {
        val event = d2.eventModule().events().uid(eventUid).blockingGet() ?: return false
        val programUid = event.program().trimToValue() ?: return false
        val programStageUid = event.programStage().trimToValue() ?: return false

        return rampDatastoreRepository
            .getConfig()
            .programStageHistoryTables
            .any { config ->
                config.programId.trimToValue() == programUid &&
                    config.followUpVisitProgramStageId.trimToValue() == programStageUid
            }
    }
